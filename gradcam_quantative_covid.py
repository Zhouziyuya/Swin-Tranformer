...
def parse_option():
    ...
    parser.add_argument('--backbone', type=str, default='swin_base', help='swin_base, swin_large, vit_base, resnet50, vit_base_patchsize16, convnext, ')
    parser.add_argument('--dataset', type=str, default='CovidQuEx', help='Chestxray14, RSNA, SIIM, CovidQuEx')
    parser.add_argument('--model_type', type=str, default='swin', help='swin, swinv2')
    parser.add_argument('--pretrain_mode', type=str, default='FoundationX', help='ACEv2_swinv2_large, ACEv2_swinv2, RAD-DINO, adam-v2, eva-x, ark_plus, CheXWorld, Lamps_large_swinv1, FoundationX')
    ...
    # parser.add_argument('--pretrain_weight', type=str, default='/sda/zhouziyu/ssl/downstream_checkpoints/NIHchest/RAD-DINOvit_base_patchsize16_518_3/best.pth')
    # parser.add_argument('--pretrain_weight', type=str, default='/sda/zhouziyu/ssl/downstream_checkpoints/RSNA/adam-v2convnext_linearprob_224_5/best.pth')
    # parser.add_argument('--pretrain_weight', type=str, default='/sda/zhouziyu/ssl/downstream_checkpoints/SIIM/eva-xvit_base_patchsize16_linearprob_224_5/best.pth')
    parser.add_argument('--pretrain_weight', type=str, default='/sda/zhouziyu/ssl/downstream_checkpoints/CovidQuEx/FoundationXswin_base_linearprob_224_1/best.pth')
    ...
    parser.add_argument('--bbox_file', default='/sda/zhouziyu/ssl/datasets/ChestXray/COVID-QU-Ex/Infection Segmentation Data/Test/COVID-19/infection masks/', type=str, metavar='PATH')
    parser.add_argument('--test_path', default='/sda/zhouziyu/ssl/datasets/ChestXray/COVID-QU-Ex/Infection Segmentation Data/Test/COVID-19/images/', type=str, metavar='PATH')
    # parser.add_argument('--correp_file', default='/sda/zhouziyu/ssl/datasets/SIIM/stage_1_test_images.csv', type=str, metavar='PATH')
    # parser.add_argument('--out_path', default='/nvme1n1/zhouziyu/Swin-Transformer/figures/gradcam/Adamv2/RSNA/run5', type=str, metavar='PATH')
    parser.add_argument('--out_path', default='/nvme1n1/zhouziyu/Swin-Transformer/figures/gradcam/FoundationX/CovidQuEx/run1', type=str, metavar='PATH')
    # parser.add_argument('--out_path', default='/nvme1n1/zhouziyu/Swin-Transformer/figures/gradcam/FoundationX/', type=str, metavar='PATH')
    ...
    # parser.add_argument('--test_label', default='/sda/zhouziyu/ssl/datasets/SIIM/stage_1_test_images.csv', type=str, metavar='PATH')


    # distributed training
    parser.add_argument("--device", type=int, default=1, help='local rank for DistributedDataParallel')
...
def preprocess_img(args, img_name, device):
    ...
    # ds = dicom.dcmread(os.path.join(args.test_path, img_name))
...
def draw_mask_contour(img_path, mask, img_size,
                      color=(255, 255, 255),
    ...
    Args:
        ...
        img_size (int): resized image size (e.g., 224, 768)
        color (tuple): BGR color
        original_size (bool): whether mask is normalized to 1024 scale
    """
    w, h = original_size
    image = cv2.imread(img_path)
...
def main():
    ...
    # 写入标题行
    writer.writerow(['Image Index', 'X', 'Y', 'W', 'H', 'confidence', 'class', 'box label'])
    ...
    if args.dataset == 'SIIM':
        ...
        df = pd.read_csv(args.test_label)
        pneumo_df = df[df['has_pneumo'] == 1]
        
        # 构建test_labels字典 (key: new_filename, value: 2)
        test_labels = {row['new_filename']: 1 for _, row in pneumo_df.iterrows()}
        
        patient_ids = [fname for fname in test_labels.keys()]
        image_indices = sorted(set(patient_ids))  # 去重并排序
        ...
        image_indices = os.listdir(args.test_path)
        
        disease_map = ['Normal', 'Covid', 'Non-Covid'] 
        num_classes = len(disease_map)
        label = 2
        ...
        disease_map = ['normal', 'No Lung Opacity/Not normal', 'Lung Opacity']
        num_classes = len(disease_map)
        ...
        # For RSNA filter by patient ids present in test_labels
        # test_labels keys are filenames like 'xxxx.png' — strip extension to get patientId
        patient_ids = set([k.split('.')[0] for k in test_labels.keys()])
        df['patientId'] = df['patientId'].astype(str)
        df = df[df['patientId'].isin(patient_ids)]
        image_indices = sorted(df['patientId'].unique())
    ...
    for cls_idx, cls_name in enumerate(disease_map):
        ...
        if cls_name == 'Normal' or cls_name == 'Non-Covid':
            continue
    
        for img_idx in image_indices:
            img_name = img_idx
            
            input_tensor, rgb_img_norm, w, h = preprocess_img(args, img_idx, device)
            ...
            conflist = output[0]
            writer2.writerow([img_name, conflist])
            
            
            multi_hot = np.zeros(num_classes, dtype=np.uint8)

            # img_annotations = df[df['patientId'] == img_idx]
            mask = cv2.imread(os.path.join(args.bbox_file, img_idx))
            # mask[mask==255]=1
            label = int(np.any(mask) != 0 )# disease label 0/1
            ...
            # gt information
            # cls_annotations = img_annotations[img_annotations['Target'] == 1]
            # ipdb.set_trace()
            ...
            #     bbox_cols = [row.iloc[1], row.iloc[2], row.iloc[3], row.iloc[4]]
            ...
            draw_mask_contour(os.path.join(save_path_red_heatmap_with_gtbox, cls_name, img_name), mask[:,:,0], args.img_size, original_size=(w,h)) # draw gt mask contour
            ...
            mask_resize = cv2.resize(mask[:,:,0], (args.img_size, args.img_size))
            ...
            # bbox_mask = cv2.resize(mask, (args.img_size, args.img_size))
            gt_binary = (mask_resize >= 0.5).astype(np.float32)
            gt_seg_maps.append(gt_binary)
            ...
            if gradcam_box is not None:
                writer.writerow([img_name,*gradcam_box, pred_conf, cls_idx, multi_hot[cls_idx]])
